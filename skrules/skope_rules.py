...
from sklearn.tree import _tree  # Note: This is a private API
...
def indices_to_mask(indices, size):
    ...
    indices : array-like of int
        Indices to set to True in the mask.
    size : int
        The size of the resulting mask.

    Returns
    -------
    mask : ndarray of bool
        Boolean mask with True at the specified indices.
    """
...
class SkopeRules(BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if not set(self.classes_) == {0, 1}:
            warn(f"Found labels {set(self.classes_)}. This method assumes target class "
                 "to be labeled as 1 and normal data to be labeled as 0. Any label "
                 "different from 0 will be considered as being from the target class.")
            y = (y > 0)
        ...
        if isinstance(self.max_samples, str):
            ...
            if self.max_samples > n_samples:
                warn(f"max_samples ({self.max_samples}) is greater than the "
        ...
        feature_names_ = [BASE_FEATURE_NAME + str(x) for x in
        ...
        if sample_weight is not None:
            ...
            y_reg = (
                np.sqrt(weights) * 0.5 / contamination * (y > 0) -
                np.sqrt(weights.mean()) * (y == 0)
            )
            y_reg = 1. / (1 + np.exp(-y_reg))  # sigmoid
        else:
            y_reg = y  # same as another classification bagging
        ...
        for estimator, samples, features in zip(self.estimators_,
            ...
            if X_oob.shape[1] > 1:  # Avoid pandas bug (cf. issue #16363)
                y_oob = y[mask]
        ...
        for rule, score in rules_:
            if score[0] >= self.precision_min and score[1] >= self.recall_min:
                if rule in self.rules_:
                    ...
                    b = self.rules_[rule][1] + (score[1] - self.rules_[rule][1]) / c
                    a = self.rules_[rule][0] + (score[0] - self.rules_[rule][0]) / c
        ...
        # Deduplicate the rules using semantic tree
        ...
        # Replace generic feature names with real feature names
    ...
    def predict(self, X):
        ...
        is_outlier : array, shape (n_samples,)
            For each observation, tells whether or not (1 or 0) it should
            be considered as an outlier according to the selected rules.
        """
        return np.array((self.decision_function(X) > 0), dtype=int)

    def decision_function(self, X):
        """Average anomaly score of X from the base classifiers (rules).
        ...
        if X.shape[1] != self.n_features_:
            raise ValueError(f"X.shape[1] = {X.shape[1]} should be equal to {self.n_features_}, "
                             "the number of features at training time. Please reshape your data.")
        ...
        for (r, w) in selected_rules:
            matched_indices = df.query(r).index
            scores[matched_indices] += w[0]
    ...
    def score_top_rules(self, X):
        ...
        for (k, r) in enumerate(selected_rules):
            matched_indices = df.query(r[0]).index
            scores[matched_indices] = np.maximum(
    ...
    def _tree_to_rules(self, tree, feature_names):
        ...
        def recurse(node, base_name):
            if tree_.feature[node] != _tree.TREE_UNDEFINED:
                ...
                left_rule = base_name + [f"{name} {symbol} {threshold}"]
                recurse(tree_.children_left[node], left_rule)

                right_rule = base_name + [f"{name} {symbol2} {threshold}"]
                recurse(tree_.children_right[node], right_rule)
            else:
                rule = ' and '.join(base_name) if base_name else f"{feature_names[0]} == {feature_names[0]}"
                rules.append(rule)

        recurse(0, [])

        return rules if rules else ['True']

    def _eval_rule_perf(self, rule, X, y):
        matched = X.query(rule)
        detected_index = matched.index.tolist()
        if len(detected_index) <= 1:
            return (0, 0)
        y_detected = y[detected_index]
        true_pos = y_detected.sum()
        if true_pos == 0:
            return (0, 0)
        pos = y.sum()
    ...
    def _find_similar_rulesets(self, rules):
        ...
        rules : List of tuples
                The rules that should be split into subsets of similar rules.

        Returns
        -------
        List of lists of rules
                The different sets of rules. Each set should be homogeneous.
        """
        def split_with_best_feature(rules, depth, exceptions=[]):
            """
            Method to find a split of rules given the most represented feature.
            """
            if depth == 0:
                return rules
            ...
            most_represented_term = counter.most_common(1)[0][0]
            # Proceed to split
            rules_splitted = [[], [], []]
            for rule in rules:
                if f"{most_represented_term} <=" in rule[0]:
                    rules_splitted[0].append(rule)
                elif f"{most_represented_term} >" in rule[0]:
        ...
        leaves = breadth_first_search(res)
        return leaves
